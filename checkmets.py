...
for root, dirs, files in os.walk(basedir):
    for file in files:
        if file.endswith(".xml"):
            ...
            for md5s in doc.xpathEval("//@CHECKSUM"):
                md5list.append(md5s.content)
            for ref in context.xpathEval('//@xlink:href'):
                # figure out how one can avoid echo -n to not have b and  \n (new lines and breaks) in output
                cal_md5 = subprocess.Popen(["echo -n $(md5sum %s | cut -d' ' -f 1)" % ref.content],
                                           stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                           shell=True, universal_newlines=True)
                (out, err) = cal_md5.communicate()
                reflist.append(out.upper())
                file_to_check = os.path.join(directory, ref.content)
